...
import { getAudioMedia, getMedia } from "../utils/media"
...
export const useWebcamStream = ({
    ...
    const handleGetMedia = useCallback(
        (
            ...
            >,
            pendingSignalsRef: React.RefObject<
    ...
    const handleToggleMic = useCallback(
        async (
            ...
            if (!streamRef.current && !micOn) {
                const started = await getAudioMedia(
                    selectedAudioInput,
                    streamRef,
                    peersRef,
                    setRemoteStreams,
                ...
                if (started) {
                    setMicOn(true)
                    socket.emit(SocketEvent.MIC_STATE, true)
                }
                return
            }
            toggleMic(micOn, setMicOn, streamRef)
        },
        [micOn, selectedAudioInput, setMicOn, socket],
    )

    const handleToggleSpeaker = useCallback(
        (newState: boolean) => {
            setSpeakerOn(newState)
            socket.emit(SocketEvent.SPEAKER_STATE, newState)
